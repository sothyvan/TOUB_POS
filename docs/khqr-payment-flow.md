...
| :--- | :--- | :--- | :--- |
| [order.service.js](file:///d:/CADT/VSCODE/Final_Project_Y2/TOUB_POS/backend/src/services/order.service.js) | Order Service | `createOrder(cashierId, items, paymentMethod)` | Handles MySQL transactions: calculates totals based on DB catalog records, constructs the order items, invokes KHQR generator if method is `khqr`, logs creation audit trail, and commits. |
| [order.service.js](file:///d:/CADT/VSCODE/Final_Project_Y2/TOUB_POS/backend/src/services/order.service.js) | Order Service | `checkKhqrPaymentStatus(orderId, actor)` | Public service API to fetch and evaluate transaction status from Bakong with user permission checking. |
| [order.service.js](file:///d:/CADT/VSCODE/Final_Project_Y2/TOUB_POS/backend/src/services/order.service.js) | Order Service | `checkKhqrPaymentStatusAsSystem(orderId)` | System context caller enabling verification without user-level access restrictions. |
| [order.service.js](file:///d:/CADT/VSCODE/Final_Project_Y2/TOUB_POS/backend/src/services/order.service.js) | Order Service | `checkKhqrPaymentStatusInternal(...)` | Compares locally stored state with Bakong API, updates DB to `paid` if correct, invokes WebSocket emitter, and enqueues Telegram notifications. |
...
| [khqr-background-checker.service.js](file:///d:/CADT/VSCODE/Final_Project_Y2/TOUB_POS/backend/src/services/khqr-background-checker.service.js) | Background Worker | `startKhqrBackgroundChecker()` | Starts standard javascript `setInterval` loop to query unpaid transactions on the background. |
| [khqr-background-checker.service.js](file:///d:/CADT/VSCODE/Final_Project_Y2/TOUB_POS/backend/src/services/khqr-background-checker.service.js) | Background Worker | `runKhqrBackgroundCheckOnce()` | Queries pending, unexpired KHQR database entries and triggers checking tasks sequentially. |
