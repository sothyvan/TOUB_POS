...
-- Preflight used by the managed Telegram routing uniqueness migration.
-- Any returned row requires operator cleanup; the migration never chooses a
-- winning Stall or rewrites a kitchen destination automatically.
...
-- `active_telegram_chat_id` is generated by MySQL and must never be written by
-- Sequelize/application queries. Its unique index permits only one non-deleted
-- Stall per Telegram kitchen chat while allowing reuse after soft deletion.
