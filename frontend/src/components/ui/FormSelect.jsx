  ...
  return (
    <label className={wrapperClassName}>
      ...
      <select
        ...
        className={`w-full min-h-11.5 px-3.5 border rounded-lg bg-ui-surface text-brand-text text-[14px] font-medium outline-none transition-all disabled:cursor-not-allowed disabled:bg-ui-muted disabled:text-text-muted ${error ? 'border-state-danger focus:border-state-danger focus:ring-1 focus:ring-state-danger' : 'border-brand-border focus:border-brand-action focus:ring-1 focus:ring-brand-action'} ${className}`}
      >
        {children}
      </select>
      {error || helperText ? (
