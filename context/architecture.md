...
- **Database backups**: Database dumps are never source artifacts and must not be tracked by Git. The repository backup command writes plaintext only to an operating-system temporary directory, retains an AES-256 GPG-encrypted artifact outside version control, and requires a separate protected encryption passphrase. CI allows SQL only in the canonical migration and course-document paths.
