...
CREATE TABLE business_financial_settings (
  owner_id INT PRIMARY KEY,
  exchange_rate_khr_per_usd INT NOT NULL DEFAULT 4100,
...
CREATE TABLE orders (
  ...
  subtotal_khr   INT NOT NULL DEFAULT 0,               -- trusted sum of KHR item snapshots
  total_khr      INT NOT NULL DEFAULT 0,               -- trusted KHR final total
  pricing_currency ENUM('usd', 'khr') NOT NULL DEFAULT 'usd',
  exchange_rate_khr_per_usd INT NOT NULL DEFAULT 4100, -- immutable order snapshot
  cash_received_usd DECIMAL(10, 2) DEFAULT NULL,       -- cash handed over by customer; cash orders only
  cash_received_khr INT DEFAULT NULL,
  change_due_usd DECIMAL(10, 2) DEFAULT NULL,          -- backend-calculated cash change; cash orders only
  change_due_khr INT DEFAULT NULL,
  change_currency ENUM('usd', 'khr') DEFAULT NULL,    -- legacy selected denomination; NULL for new dual-display change
